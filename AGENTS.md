...
- Keep the Hockeymom configuration dimension guide synchronized: after changing `cooler-gopro/hockeymom_3_cam_cover_original_style_blender.py`, `cooler-gopro/gopro_mission1_dummy_blender.py`, or `cooler-gopro/generate_hockeymom_config_dimension_pdf.py`, run `make -C cooler-gopro dim-pdf` followed by `make -C cooler-gopro check-dim-pdf-sync`. Do not leave the local generated PDF stale even though `*.pdf` is Git-ignored.
