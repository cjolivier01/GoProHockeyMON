...
- Do not use the word `Codex` or refer to any AI agent in pull request titles, descriptions, comments, review requests, or branch names.
...
- Preserve the Mission 1 field case internal width, height, and depth across incremental revisions. Do not change any of those three dimensions without explicit user approval; keep latch, handle, mount, and insert refinements compatible with already-printed case parts whenever possible.
- Keep the Hockeymom configuration dimension guide synchronized: after changing `models3d/hockeymon-camera-case/hockeymom_cam_case_blender.py`, `models3d/mission1-dummy/gopro_mission1_dummy_blender.py`, or `models3d/hockeymon-camera-case/generate_hockeymom_cam_case_dimension_pdf.py`, run `make -C models3d dim-pdf` followed by `make -C models3d check-dim-pdf-sync`. Do not leave the local generated PDF stale even though `*.pdf` is Git-ignored.
- Keep generated print projects synchronized with model changes: update the 3MF generation whenever geometry, part selection, placement, or material assignments change; regenerate and validate the complete affected 3MF and STL exports, and refresh its plate overview before opening or updating a PR. Do not leave local generated artifacts stale even when Git-ignored.
