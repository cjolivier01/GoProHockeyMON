...
def unit_for(name: str, value: object) -> str:
    ...
    if name in DIMENSIONLESS_NUMERIC_NAMES:
        return "setting"
    if name in {"TPU_RECOMMENDED_INFILL_PERCENT", "TPU_RECOMMENDED_WALLS"}:
        return "setting"
...
def draw_feature_annotation(ax, view, entry, index, bounds, color):
    ...
    if view == "assembly_side" and entry.identity == "TPU_MOUNT_SCREW_EXTRA_LENGTH_MM":
        data = adapter_datums()
...
def page_profiles_overview(pdf, page_number: int):
    ...
    ax.text(0.81, 0.97, "TPU", fontsize=5.8, weight="bold", color=ORANGE)
    for index, key in enumerate(keys):
        ...
        ax.text(0.81, y, fmt(profile["TPU"].get(key)), fontsize=5.4, color=ORANGE, va="center")
